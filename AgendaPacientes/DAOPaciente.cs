...
namespace AgendaPacientes
{
    class DAOPaciente
    {
        ...
        public long[] cpfVet;
        public double[] dividaVet;
        public DateTime[] horarioVet;
        ...
        public void Inserir(string nome, string telefone, string convenio, string tratamento,
        string lugar, long cpf, double divida, DateTime horario, string dias, string imposto)
        {
            try
            {
                //Preparar os dados para inserir no banco
                dados = "('','" + nome + "','" + telefone + "','" + convenio + "','" + tratamento +
                "','" + lugar + "','" + cpf + "','" + divida + "','" + horario + "','" +
                dias + "','" + imposto + "')";
                comando = "Insert into agenda(codigoPaciente, nome, telefone, convenio,, tratamento, lugar, cpf, divida, horario, dias, imposto) values " + dados;
        ...
        {
            string query = "select * from agenda";//comand para coletar todos os dados do banco
            ...
            cpfVet = new long[100];
            dividaVet = new double[100];
            horarioVet = new DateTime[100];
            ...
            while (leitura.Read())//enquanto leitura for verdadeiro executa while
            {
                ...
                cpfVet[i] = Convert.ToInt64(leitura["cpf"]);
                dividaVet[i] = Convert.ToDouble(leitura["divida"]);
                horarioVet[i] = Convert.ToDateTime(leitura["horario"]);
        ...
        public double ConsultarDivida(int cod)
        {
            PreencherVetor();
            for (i = 0; i < contador; i++)
            {
        ...
        public DateTime ConsultarHorario(int cod)
        {
            PreencherVetor();
            for (i = 0; i < contador; i++)
            {
        ...
        public string Atualizar(int codigo, string campo, string novoDado)
        {
            try
            {
                string query = " update pessoaTI14T set " + campo + " = " + novoDado + "' where codigo = '" + codigo + "'";
        ...
        public void Excluir(int codigo)
        {
            try
            {
                string query = "delete from agenda where codigoPaciente = '" + codigo + "'";
                MySqlCommand sql = new MySqlCommand(query, conexao);
                string resultado = "" + sql.ExecuteNonQuery();
